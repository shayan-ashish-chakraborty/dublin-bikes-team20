    ...
    try:
        ...
        result = db_session.execute(
            text(
                ...
                SELECT *
                FROM hourly
                WHERE future_dt >= UTC_TIMESTAMP() - INTERVAL 1 HOUR
...
def openweather_current(lat: float, lon: float) -> dict:
    ...
    and normalize into our frontend-friendly shape.
    """
    cfg = Config()
...
def hourly_forecast_list_like_weather_page(limit: int = 24) -> list[dict]:
    ...
    if len(rows) < _MIN_HOURLY_ROWS:
        try:
            ...
            if db_rows:
                # API rows are preferred per 3h slot; pass them as the "db" argument so they win
